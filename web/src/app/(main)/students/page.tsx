...
export default function StudentsPage() {
  ...
  const load = useCallback(async () => {
    setErr(null);
    try {
      const r = await api.get<PersonItem[]>("/api/students");
      setRows(r.data);
    } catch (e) {
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      await api.post("/api/students", { fullName, email });
      setFullName("");
      setEmail("");
      await load();
    } catch (e) {
  ...
  async function onSaveEdit(e: React.FormEvent) {
    ...
    setErr(null);
    try {
      await api.patch(`/api/students/${editId}`, { fullName: editName, email: editEmail });
      setEditOpen(false);
      await load();
    } catch (e) {
      setErr(getAxiosErrorMessage(e));
    ...
    <div>
      ...
      >
        <div className="flex flex-1 min-w-[160px] flex-col gap-1">
          <label className="text-xs text-zinc-500" htmlFor="s-name">
            Full name
          </label>
          <input
            id="s-name"
        ...
        <div className="flex flex-1 min-w-[200px] flex-col gap-1">
          <label className="text-xs text-zinc-500" htmlFor="s-email">
            Email
          </label>
          <input
            id="s-email"
